...
def main():
    parser = argparse.ArgumentParser(
        description="dokan: an automated NNLOJET workflow")
    ...
    parser_init.add_argument('-o',
                             '--output',
                             dest='job_path',
                             help='destination of the job directory')
    ...
    parser_submit.add_argument('--policy',
    ...
    if args.action == 'init':
        ...
        if args.job_path is not None:
            ...
            dokan.CONFIG.set_job_path(os.path.relpath(
                runcard_data["job_name"]))
        dokan.CONFIG.set_job(name=runcard_data["job_name"])
        dokan.CONFIG.set_process(name=runcard_data["process_name"])
        dokan.CONFIG.set_process(channels=dokan.nnlojet.get_lumi(
