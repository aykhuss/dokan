...
class Production(Task):
    ...
    def requires(self):
        return Warmup(
            config=self.config, local_path=self.local_path, channel=self.channel
        )
    ...
    def _append_production(self) -> dict:
        ...
        if len(self._data) > 0:
            ...
            for _, prod in filter(
                lambda item: item[1][1] is not None, self._data.items()
            ):
                nprod += 1
            ...
            avg_time_per_evt: float = sumt / float(ntot)
            err_time_per_evt: float = math.sqrt(
                (sumt2 - sumt**2 / float(ntot)) / float(ntot)
            )
            # > exactly hit min_runtime
            ncall_min: int = int(
                self.config["job"]["min_runtime"] / avg_time_per_evt / float(niter)
            )
            # > aim for max_rumtime with a 5-sigma buffer
            ncall_max: int = int(
                self.config["job"]["max_runtime"]
                / (avg_time_per_evt + 5.0 * err_time_per_evt)
                / float(niter)
            )
        ...
        exe_args = dict(
            local_path=[*self.local_path, self._idir(self.iseed)],
