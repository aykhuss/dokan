...
class LocalExec(Executor):
    ...
    def exe(self):
        ...
        start_time = time()
        with open(self._local(Executor._file_out),
                  'w') as of, open(self._local(Executor._file_err), 'w') as ef:
        ...
        with open(self._local(Executor._file_out), 'r') as of:
            iteration = {}
            for line in of:
                match_iteration = re.search(r'\(\s*iteration\s+(\d+)\s*\)',
                                            line, re.IGNORECASE)
                if match_iteration:
                ...
                match_chi2it = re.search(r'\schi\*\*2/iteration\s*=\s*(\S+)\b',
                                         line, re.IGNORECASE)
                if match_chi2it:
        ...
        for file in os.listdir(self._path):
            if file in [
                    ...
                    Executor._file_out, Executor._file_err
            ]:
                continue
