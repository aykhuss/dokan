...
class Warmup(Task):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.channel not in self.config["process"]["channels"]:
            raise RuntimeError("Warmup: unknown channel: {}".format(self.channel))
        ...
        if not self._done() and os.path.exists(self._local(Warmup._file_res)):
    ...
    def save_data(self, move_to_res: bool = False):
        ...
        if move_to_res:
            shutil.move(self._local(Warmup._file_tmp), self._local(Warmup._file_res))
    ...
    def _done(self) -> bool:
        ...
        test_accuracy = (
            abs(self._data[-1][1]["error"] / self._data[-1][1]["integral"])
            < target_rel_accuracy
        )
    ...
    def _append_warmup(self) -> dict:
        ...
        if len(self._data) > 0:
            ...
            ncall = int(
                last_warmup[0]["ncall"] * self.config["warmup"]["fac_increment"]
            )
        ...
        exe_args = dict(
            local_path=[*self.local_path, "w{}".format(iseed)],
